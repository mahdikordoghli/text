...
<body>
    ...
    <main id="main" class="main">
        ...
        <?php
        $sql = "SELECT * FROM assurance WHERE matricule = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("s", $mat);
        $stmt->execute();
        $res = $stmt->get_result();
        if (mysqli_num_rows($res) != 0) {
            $sql2 = "SELECT nom, prenom FROM userinfo WHERE matricule = ?";
            $stmt2 = $mysqli->prepare($sql2);
            $stmt2->bind_param("s", $mat);
            $stmt2->execute();
                ...
                <div class="row">
                                ...
                                <table class='table'>
                                    ...
                                    while ($row = $res->fetch_array()) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <button class="btn btn-primary consulte"
                                                    data-id="<?php echo $row['id']; ?>">Consulter</button>
                                            </td>
                                        </tr>
                                        <?php
                                    } ?>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            document.querySelectorAll('.consulte').forEach(function (button) {
                                button.addEventListener('click', function () {
                                    var id = this.getAttribute('data-id');
                                    const encodedId = new URLSearchParams({ id }).toString();
